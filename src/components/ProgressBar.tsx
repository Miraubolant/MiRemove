...
export function ProgressBar({ total, completed, maxFreeImages }: ProgressBarProps) {
  ...
  // Réinitialiser le temps de départ quand un nouveau batch commence
  useEffect(() => {
  ...
  const estimatedTimeRemaining = useMemo(() => {
    ...
    // Seulement pour une session active, utiliser un temps par défaut raisonnable si pas de statistiques fiables
    if (validCompleted === 0 && total > 0) {
      // Pour une nouvelle session, utiliser 5 secondes par image comme estimation par défaut
      return Math.round(remaining * 5);
    }
    
    // Si on a des stats de session en cours, les utiliser
    if (validCompleted > 0 && elapsedTime > 0) {
      const currentAverageTime = elapsedTime / validCompleted; // Temps moyen actuel en secondes
      return Math.round(remaining * currentAverageTime);
    }
    
    // Sinon utiliser les stats générales mais avec un maximum raisonnable
    const avgTimePerImageMs = stats.averageProcessingTime;
    const avgTimePerImageSec = avgTimePerImageMs / 1000;
    
    // Limiter à maximum 30 secondes par image pour éviter des estimations absurdes
    const reasonableAvgTime = Math.min(avgTimePerImageSec, 30);
    
    return Math.round(remaining * reasonableAvgTime);
