/*
  # Group Management System - Policy Updates
  
  This migration adds additional checks to ensure policies are created only if they don't exist.
*/
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Admins can manage groups'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Admins can manage group members'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'group_members' 
    AND policyname = 'Users can view their group memberships'
  ) THEN
...
-- Update or create group stats function
...
-- Create initial Arlettie group if it doesn't exist
